...
public class UserController extends BaseController {
    ...
    public String edit(Model model, Long id ) {
    	if ( id != null ) {
		    ...
		    List<UserRole> userRList = userRoleService.selectByMap(map);
		    if(null!=userRList&&userRList.size()>0){
		    	model.addAttribute("existRole",userRList.get(0) );
		    }
	        List<UserGroup> usergList = userGroupService.selectByMap(map);
		    if(null!=usergList&&usergList.size()>0){
			  model.addAttribute("existGroup",usergList.get(0) );
		    }
		} 
		model.addAttribute("roleList", roleService.selectList(null));
		model.addAttribute("groupList", groupService.selectList(null));
	...
	public String editUser( User user ) {
		...
		if ( user != null ) {
			...
			if ( user.getId() != null ) {
				...
				user.setLastTime(existUser.getLastTime()); 
				user.setCrTime(existUser.getCrTime()); 
				rlt = userService.updateUser(user);
			} else {
